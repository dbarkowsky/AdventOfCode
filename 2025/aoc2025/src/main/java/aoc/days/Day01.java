...
public class Day01 {
  ...
  public static void Part2(ArrayList<String> input) {
    ...
    for (String line : input) {
      ...
      if (direction.equals("R")) {
        // Right is easy. We just look at how many times the whole thing can be divided by the dial size.
        zeroCount += (currentValue + steps) / 100;
        currentValue = (currentValue + reducedMovement) % 100;
      } else {
        // Left sucks. 
        // We don't want to count the case where we start at 0 then move left as a roll-over. 
